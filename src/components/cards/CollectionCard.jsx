...
const CollectionCard = ({ Cover, Logo, Name, Description, OwnerAddress, CollectionAddress }) => {
    return (
        <div className="rounded-2.5xl border border-jacarta-100 bg-white p-[1.1875rem] transition-shadow hover:shadow-lg dark:border-jacarta-700 dark:bg-jacarta-700">
            <Link href={`/collection/${CollectionAddress}`} className="relative flex space-x-[0.625rem]">
                <span className="w-[100%]">
                    <Image
                        ...
                        alt="Cover Image"
                        className="h-[120px] w-[100%] rounded-[0.625rem] object-cover"
                        loading="lazy"
                        height={100}
                        width={100}
                    />
            ...
            <Link href={`/collection/${CollectionAddress}`} className="mt-8 block font-display text-[22px] text-jacarta-700 hover:text-accent dark:text-white dark:hover:text-accent">
                {Name}
            </Link>
